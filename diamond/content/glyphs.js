...
(function (root) {
  ...
  G['hero'] = {
    ...
    center: { run: true },
    captions: {
      pitches: '左の細い欄に、投げられた球を上から1球ずつ。●ボール ×見逃し 空振りは×に斜線 △ファウル □打った',
  ...
  /* ---- 交代の書き方（v1.9・パ・リーグ.comの記入例の図で確定）----
     交代は「打順欄」と「マスの境目の波線」の2か所に書く。難しい単元こそ
     具体例を見せる（2026-07-30 本人FB）。選手名は names.js の創作名のみ。 */
  ...
  G['lineup-sub'] = {
    ...
    lineup: [
      {no:'9',  pos:'2', name:'つばき'},
      {no:'19', mark:'PH', name:'かなで', pen:'ink'},
      {no:'29', mark:'PR', pos:'6', name:'そらち', pen:'ink'}
    ],
    captions: {
      lineup: '打順は9つのまま。同じ打順の中に 19・29 と行を足して、PH（代打）・PR（代走）と選手名を書く。守備についたら守備番号も（PRのそらちは遊撃＝6）'
    }
  };
  ...
  G['ph-atbat'] = {
    ...
    result: { text: 'SO' },
    center: { out: 2 },
    sub: { at:'left', label:'PH かなで' },
    captions: {
      sub: 'マスの左の境目に波線を引き、脇に PH と代打の選手名。ここから先は代打の記録',
      result: '代打は見逃し三振。打席の結果は、いつもどおり右下に',
      center: 'この回2つ目のアウトなのでⅡ'
    }
  };

  G['pr-onbase'] = {
    title: '出塁したあと代走を出したマス',
    kind: 'walk',
    pitches: ['B','S','B','B','B'],
    result: { text: 'B' },
    sub: { at:'right', label:'PR そらち' },
    captions: {
      pitches: 'ボール4つ',
      result: '四球で出塁（青いB）。斜線は引かない',
      sub: 'マスの右の境目に波線を引き、脇に PR と代走の選手名。出塁したあとの交代なので右側'
    }
  };

  G['fielder-change'] = {
    title: '守備の交代（相手の攻撃側のマス）',
    result: { text: '1-3' },
    center: { out: 1 },
    sub: { at:'top', label:'1 ことり ／ 3 ひなた' },
    captions: {
      sub: '守備の交代は【相手の攻撃を書いている側】のマスの上端に横の波線を引き、その上に「守備番号＋名前」。投手（1）がことり、一塁手（3）がひなたに代わった',
      result: 'この打者は投手（1）が捕って一塁（3）でアウト',
      center: 'この回の1アウト目'
    }
  };
